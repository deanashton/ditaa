...
public class ShapeEdge {
	...
	public ShapePoint getMiddle() {
		return new ShapePoint((startPoint.x + endPoint.x) / 2, (startPoint.y + endPoint.y) / 2);
	}

	/**
	 * Returns the type of the edge (<code>TYPE_HORIZONTAL, TYPE_VERTICAL, TYPE_SLOPED).
	 * 
	 * @return
	 */
	private int getType() {
	...
	public boolean equals(Object object) {
		...
		if (startPoint.equals(edge.getStartPoint()) && endPoint.equals(edge.getEndPoint())) {
			return true;
		}
		if (startPoint.equals(edge.getEndPoint()) && endPoint.equals(edge.getStartPoint())) {
			return true;
		}
		return false;
	}
	...
	public boolean touchesWith(ShapeEdge other) {
		...
		if (other.isHorizontal() && isVertical()) {
			return false;
		}

		if (getDistanceFromOrigin() != other.getDistanceFromOrigin()) {
			return false;
		}
		...
		if (startPoint.isWithinEdge(other) || endPoint.isWithinEdge(other)) {
			return true;
		}
	...
	/**
	 * if horizontal flips start and end points so that start is left of end if verical flips start and end points so that start is over of end
	 * 
	 */
